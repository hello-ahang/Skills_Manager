...
import { runSandboxTest, autoGenerateCases, loadSandboxHistory, saveSandboxHistory, clearSandboxHistory, aggregateResults, } from '../services/sandboxService.js';
const router = Router();
/**
...
router.post('/test', async (req, res) => {
    try {
        ...
        const validMode = mode === 'manual' || mode === 'ai-generated' ? mode : undefined;
        ...
        const validCases = cases
            .filter((c) => c && typeof c.scenario === 'string' && typeof c.expectedSkill === 'string')
            .map((c) => ({ scenario: c.scenario.trim(), expectedSkill: c.expectedSkill.trim() }));
 ...
 */
router.post('/auto-generate-cases', async (req, res) => {
    try {
        ...
        const cases = await autoGenerateCases({ name: skill.name, description: skill.description, contentSummary: skill.contentSummary }, { baseUrl, apiKey, modelName }, Math.min(Math.max(count, 1), 10));
        res.json({ cases });
    }
    catch (error) {
