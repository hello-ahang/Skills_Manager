...
import { scanGitHub, scanGitee, scanGitLab, scanBitbucket, scanClawHub, scanLocal, scanZip, scanClipboard, executeImport, importBatch, importFromCSV, importFromJSON, exportToCSV, exportToJSON, checkConflicts, cleanupTempFiles, detectUrlType, getImportProviders, detectProvider, } from '../services/importService.js';
...
router.get('/git-tokens', async (_req, res) => {
    try {
...
router.put('/git-tokens', async (req, res) => {
    try {
...
router.post('/scan/provider/:providerId', async (req, res) => {
    try {
...
router.post('/scan/auto-detect', async (req, res) => {
    try {
...
router.post('/scan/github', async (req, res) => {
    try {
...
router.post('/scan/gitee', async (req, res) => {
    try {
        const { url, branch } = req.body;
        if (!url) {
...
router.post('/scan/gitlab', async (req, res) => {
    try {
        const { url, branch } = req.body;
        if (!url) {
...
router.post('/scan/bitbucket', async (req, res) => {
    try {
        const { url, branch } = req.body;
        if (!url) {
...
router.post('/scan/clawhub', async (req, res) => {
    try {
        const { url } = req.body;
        if (!url) {
...
router.post('/select-path', async (req, res) => {
    try {
        ...
        if (platform === 'darwin') {
            ...
            try {
                selectedPath = execSync(`osascript -e 'set theFolder to choose folder with prompt "选择要导入的文件夹"' -e 'POSIX path of theFolder'`, { encoding: 'utf-8', timeout: 60000 }).trim();
            }
            catch {
            ...
            try {
                selectedPath = execSync(`powershell -Command "Add-Type -AssemblyName System.Windows.Forms; $f = New-Object System.Windows.Forms.FolderBrowserDialog; $f.Description = '选择要导入的文件夹'; if ($f.ShowDialog() -eq 'OK') { $f.SelectedPath } else { '' }"`, { encoding: 'utf-8', timeout: 60000 }).trim();
            }
            catch {
                res.json({ path: '' });
                return;
            ...
            // Linux: try zenity
            try {
                selectedPath = execSync(`zenity --file-selection --directory --title="选择要导入的文件夹" 2>/dev/null`, { encoding: 'utf-8', timeout: 60000 }).trim();
            }
            catch {
                res.json({ path: '' });
                return;
...
router.post('/scan/clipboard', async (req, res) => {
    try {
...
router.post('/scan/auto', async (req, res) => {
    try {
        const { url, branch } = req.body;
        if (!url) {
            res.status(400).json({ error: 'URL is required' });
...
router.post('/execute', async (req, res) => {
    try {
        const { source, skills, options, sourceUrl } = req.body;
        ...
        const result = await executeImport(skills, options, source, sourceUrl);
...
router.post('/extensions/upload', extensionUpload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
...
router.delete('/extensions/:name', async (req, res) => {
    try {
        const extDir = path.join(os.homedir(), '.skills-manager', 'extensions');
