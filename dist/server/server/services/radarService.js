...
function parseSkillFrontmatter(content) {
    ...
    if (!result.description) {
        ...
        if (fallbackMatch)
            result.description = fallbackMatch[1].trim();
    }
    return (result.name || result.description) ? result : null;
}
/**
 ...
 */
function extractContentSummary(content, maxLen = 500) {
    // Remove frontmatter
 ...
 */
async function scanDirForSkills(dirPath, source, sourceName) {
    const skills = [];
 ...
 */
export async function aggregateAllSkills() {
    ...
    if (config.sourceDirs && config.sourceDirs.length > 0) {
        for (const dir of config.sourceDirs) {
            const dirPath = dir.path.startsWith('~/') || dir.path === '~'
                ...
                : dir.path;
            const librarySkills = await scanDirForSkills(dirPath, 'library', dir.name || 'Skills 库');
        ...
        const expandedPath = config.sourceDir.startsWith('~/') || config.sourceDir === '~'
            ...
            : config.sourceDir;
        const librarySkills = await scanDirForSkills(expandedPath, 'library', 'Skills 库');
        allSkills.push(...librarySkills);
    }
    ...
    try {
        if (await fs.pathExists(versionsIndexPath)) {
            const allVersions = await fs.readJson(versionsIndexPath);
 ...
 */
export async function saveRadarTags(tags) {
 ...
 */
export async function loadRadarSummary() {
    try {
 ...
 */
export async function saveRadarSummary(summary) {
    await fs.ensureDir(USER_CONFIG_DIR);
