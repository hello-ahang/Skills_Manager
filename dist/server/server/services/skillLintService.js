 ...
 */
export function calculateHealthScore(issues, metrics) {
    let score = 100;
 ...
 */
export async function generateHealthReport(skillName, skillDir, options) {
    ...
    if (options?.aiModelConfig && metrics.hasDescription) {
        try {
            const parsed = await parseSkillMd(skillDir);
            if (parsed?.description) {
                report.aiAssessment = await aiAssessDescription(skillName, parsed.description, options.aiModelConfig);
            }
        }
        catch {
 ...
 */
export async function getSkillDescription(skillDir) {
    const parsed = await parseSkillMd(skillDir);
 ...
 */
export async function aiAssessDescription(skillName, description, modelConfig) {
    const messages = [
        {
            role: 'system',
