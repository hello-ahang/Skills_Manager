 ...
 */
export async function syncLinks(projectIds, _tools, conflictStrategy = 'backup-replace', sourceDirId) {
 ...
 */
export async function removeLinks(projectIds, _tools, restoreAsDirectory = false) {
    const config = await getConfig();
    const results = [];
    for (const projectId of projectIds) {
        const project = config.projects.find(p => p.id === projectId);
