...
async function parseSkillMeta(dirPath) {
...
export async function buildFileTree(dirPath) {
...
export async function createNewFile(filePath, content = '') {
    if (await fs.pathExists(filePath)) {
...
export async function searchFiles(dirPath, query) {
    const results = [];
    if (!await fs.pathExists(dirPath)) {
        return results;
    }
