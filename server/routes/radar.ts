...
router.post('/search', async (req: Request, res: Response) => {
  try {
    ...
    const skillsList = skills
      .map((s: { name: string; description?: string; contentSummary?: string }) =>
        `- ${s.name}: ${s.description || s.contentSummary || '无描述'}`
      )
      .join('\n');
...
router.get('/cache/tags', async (_req: Request, res: Response) => {
  try {
...
router.put('/cache/tags', async (req: Request, res: Response) => {
  try {
    const { tags } = req.body;
...
router.get('/cache/summary', async (_req: Request, res: Response) => {
  try {
...
router.put('/cache/summary', async (req: Request, res: Response) => {
  try {
