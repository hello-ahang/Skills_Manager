...
} from '../services/skillLintService.js';
...
router.post('/batch', async (req: Request, res: Response) => {
  try {
    ...
    const skillEntries: Array<{ name: string; dir: string }> = [];
      ...
      for (let i = 0; i < reportsWithDesc.length; i += CONCURRENCY) {
        ...
        const aiResults = await Promise.allSettled(
          batch.map(async (report) => {
            try {
              ...
              if (!desc) return { skillPath: report.skillPath, aiAssessment: undefined };
        ...
        for (const result of aiResults) {
          if (result.status === 'fulfilled' && result.value.aiAssessment) {
            ...
            if (target) target.aiAssessment = result.value.aiAssessment;
          }
        }
      }
    }
 ...
 */
router.post('/ai-assess', async (req: Request, res: Response) => {
  try {
    ...
    const assessment = await aiAssessDescription(
      skillName,
      description,
      { baseUrl, apiKey, modelName }
    );
